...
var crmOmnitureRegionConfig = {
    ca: {
        omniture: {
            currencyCode: 'CAD',
            fpcDomain: 'measurement',
            fpcSecureDomain: 'smeasurement',
            accounts: {
                dev: {
                    en: {
                        nissan: 'nissancandev',
                        infiniti:'nissaninfiniticadev'
                    },
                    fr: {
                        nissan: 'nissancandev',
                        infiniti:'nissaninfiniticadev'
        ...
        // The numbers are CSIDs (Client Site ID) used by Genesis to allow DART and Omniture to cross-authenticate.
        doubleclick: {
            accounts: {
                en: '',
                fr: ''
    ...
    us: {
        omniture: {
            currencyCode: 'USD',
            fpcDomain: 'metrics',
            fpcSecureDomain: 'smetrics',
            accounts: {
                dev: {
                    en: {
                        nissan: 'devnissanusacom',
                        infiniti: 'devnissaninfiniti'
                    },
                    es: {
                        nissan: 'devnissanespanol',
                        infiniti: 'devnissaninfiniti'
                    }
                },
                prod: {
                    en: {
                        nissan: 'nissanusacom',
                        infiniti: 'nissaninfiniticom'
                    },
                    es: {
                        nissan: 'nissanespanol',
                        infiniti: 'nissaninfiniticom'
                    }
                }
            },
...
var crmOmnCurrentCfg = crmOmnitureRegionConfig[crmPageRegion];
...
function omnFlushObj() {
    for (n=1; n <= 75; n++) eval('s.prop'+n+'="";');
    for (n=1; n <= 75; n++) eval('s.eVar'+n+'="";');
...
if (console) console.log("regressiontest captured");
}
//End of SR 19115
...
var crmTools = {
    setCookie : function(name, value, opts) {
        ...
        this.mergeOptions(options, opts);
    ...
    deleteCookie : function (name, pathValue) {
        if (pathValue){
            this.setCookie(name, '', {expiryDays: -1, path: pathValue});
        } else {
...
var omnAccountSiteObj = new CrmSiteClass(crmOmnCurrentCfg.omniture.accounts[omnEnvObj.getEnv() == '' ? 'prod' : omnEnvObj.getEnv()][crmPageLocale]);
var omnDomainSiteObj = new CrmSiteClass(crmOmnCurrentCfg.omniture.domains);
...
function omnCustEvents () {
    ...
    if (s.events=='') {
        //delete crm tagging cookie
        setTimeout("crmTools.deleteCookie('NNA.previousPageCheck')",2000);
        return;
    }
...
// Prefixes are no longer overridable so says Adobe. So if envModifier is not 'us', string is assembled slightly differently
var s_account=omnAccountSiteObj.getSite()
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
/* E-commerce Config */
s.currencyCode=crmOmnCurrentCfg.omniture.currencyCode
...
s.linkInternalFilters="javascript:,nissanusa.com/,infinitiusa.com/,choosenissan.com/,nissan.ca/,infiniti.ca/,localhost"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
s.fpcdomain=omnDomainSiteObj.getSite();
s.fpcsubdomain=crmOmnCurrentCfg.omniture.fpcDomain
s.sslfpcsubdomain=crmOmnCurrentCfg.omniture.fpcSecureDomain
s.vmk="43D8135C"
...
s.trackingServer=crmOmnCurrentCfg.omniture.fpcDomain+"."+s.fpcdomain;
s.trackingServerSecure=crmOmnCurrentCfg.omniture.fpcSecureDomain+"."+s.fpcdomain;
s.dc=112
...
function s_doPlugins(s) {
	...
	// Set the appropriate campaign variables into prop38.  This is a concatenation of dcp+dcc.
    // If we do NOT have a 'dcn' parameter, then we also set these values into eVar25.  If there IS a 'dcn' parameter,
...
//begin cookie based tracking requires the NNA object to work, used to create s.prop36 and eVar4, eVar5, eVar38

    if(crmTools.getCookie('NNA.previousPageCheck')){
        s.eCookieName = 'NNA.previousPageCheck';
        s.eCookieValue = crmTools.getCookie(s.eCookieName);
        s.eCookieValueVar = s.eCookieValue.substr(s.eCookieValue.indexOf('.')+1);
        s.eVarTemp = '';
    } else {
        s.eCookieName = '';
    }
    ...
    } else if(s.eVar4 === '' && s.eCookieValue && s.eCookieValue.indexOf('promo') > -1){
       s.eVarTemp = (s.eCookieValue.indexOf('promo') > -1) ? s.eCookieValueVar : '';
       s.eVar4 = s.getValOnce(s.eVarTemp, 'evar4', 0);
       s.prop36 = s.getValOnce(s.eCookieValue,'evar4',0);
    ...
    } else if(s.eVar5=='' && s.eCookieValue && s.eCookieValue.indexOf('tool') > -1){
        s.eVarTemp = (s.eCookieValue.indexOf('tool') > -1) ? s.eCookieValueVar : '';
        s.eVar5 = s.getValOnce(s.eVarTemp, 'evar5', 0);
        s.prop36=s.getValOnce(s.eCookieValue,'evar5',0);;
    ...
    } else if(s.eVar38 === '' && s.eCookieValue && s.eCookieValue.indexOf('next') > -1){
       s.eVarTemp = (s.eCookieValue.indexOf('next') > -1) ? s.eCookieValueVar : '';
       s.eVar38 = s.getValOnce(s.eVarTemp,'next',0);
       s.prop36 = s.getValOnce(s.eCookieValue,'next',0);
...
var doubleclickAccountIdObj = new CrmLocaleClass(crmOmnCurrentCfg.doubleclick.accounts);
...
+"dc=new Date('1/1/2000');timePartingF=15;ne=8;if(dc.getDay()!=6||"
+"dc.getMonth()!=0){return'Data Not Available'}else{;z=parseInt(z);"
+"if(y=='2009'){timePartingF=8;ne=1};gmar=new Date('3/1/'+y);dsts=timePartingF-gmar.getDay("
