...
var crmSupportedRegions = {ca: ['en', 'fr']}; // available languages for each enabled country -- CANADA ONLY
...
var crmPageRegion = 'ca';           // CANADA ONLY
...
var crmProviderPath;		    // Set provider path per site
var genTag = {};                    // Create generic tagging object

var isTools = window.location.href.search(/(localhost|buildyour|byn|byi|gateway)/gi)>-1 ? true : false;		//is this a tools page?
...
function CrmSiteClass ( locSiteObj ) {
    ...
    this.getSite = function () {
        if (crmPageSite=='infiniti' || crmPageSite == 'infiniti-Canada_Undiscovered') return siteLocalObj.infiniti;
        return null;
    }
...
function CrmEnvironmentClass ( locEnvObj ) {
    ...
    this.getEnv = function () {
        ...
        // LIVE ONLY
        if (/(.nissanusa.com|.infiniti.com|.nissan.ca|.infiniti.ca)/.test(envHostName)) return envLocalEnvObj.dev;  //TODO:  temporarily set to DEV env... should be .prod when launch
...
if(typeof String.prototype.format != 'function'){
  ...
  var validCrmPageRegion = true;
  if('en' != jQuery("Script[id='crmEngine']").attr("pagelocale")) {   // CanadaUndiscovered is EN only for now
		validCrmPageRegion = false;
	}

  // ###    Validate script tag attributes. Check that the crmEngine
  if (validCrmPageRegion){

    // Check crmDebug settings and set accordingly
    crmSetDebug();

    // Set global vars from tag attributes
    crmPageLocale = jQuery('#crmEngine').attr('pagelocale');  
    crmPageSite = jQuery('#crmEngine').attr('pagesite');        
    crmPageId = jQuery('#crmEngine').attr('pageid');
    ...
    else if (!validCrmPageRegion)
        crmErrorMsg += '"pagelocale" attribute must be set to '+ crmSupportedRegions[crmPageRegion] +' in the appropriate meta tag';
    else
